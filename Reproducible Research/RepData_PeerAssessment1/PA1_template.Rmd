...
setwd("C:\\Users\\324240\\Documents\\Data Projects\\Git Repositories\\RepData_PeerAssessment1")
...
Per day, the mean number of steps  is `r as.character(round(mean(daily_steps,),0))` and the median is `r as.character(round(median(daily_steps),0))`.
...
Per day, the mean number of steps  is `r as.character(round(mean(daily_steps_fill,),0))` as compared to `r as.character(round(mean(daily_steps,),0))` with NAs, and the median is `r as.character(round(median(daily_steps_fill),0))` as compared to `r as.character(round(median(daily_steps),0))` with NAs.
